...
When a subscription upgrade or downgrade occurs, usage for the current billing period is aggregated and billed immediately.
If the target product also contains pay-per-use items, the usage tracking starts from the moment of product switch.

## One-off items

One-off items remain in the subscription during during upgrades and downgrades, and their usage data is also retained.
...
and an unmetered price with the same billing period, the billable metric's usage counter is not reset.
